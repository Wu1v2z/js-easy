// Задача: Напишіть функцію findMax, яка приймає масив чисел і повертає найбільше число в масиві.

function findMax(numbers) {
  // Якщо масив порожній — повертаємо undefined або null
  if (numbers.length === 0) {
    return undefined;
  }

  // Припустимо, що перший елемент — найбільший(ха,'жиробас' = jr)
  let max = numbers[0];

  // Перебираємо всі елементи масиву
  for (let i = 1; i < numbers.length; i++) {
    if (numbers[i] > max) {
      max = numbers[i]; // Знаходимо більше число
    }
  }

  return max; // Повертаємо найбільше(ха,print(jr)) знайдене значення
}

// Приклади використання:
console.log(findMax([3, 5, 7, 2, 8])); // Виведе: 8
console.log(findMax([10, 20, 5, 30])); // Виведе: 30

module.exports = findMax;